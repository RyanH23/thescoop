...
function createComment(url, request){
  ...
  if(request && request.body && request.body.comment){
    const requestComment = request.body.comment;
    const requestCommentBody = requestComment.body;
    const username = requestComment.username;
    let commentsArticleId = requestComment.articleId;

    if (requestCommentBody && username && commentsArticleId && database.users[username] && database.articles[commentsArticleId]){
      const comment = {
...
function saveDatabase(){
  fs.writeFile( './databasestorage/saveddatabase.yml', yaml.dump(database), function(err){
    if (err) throw err;
    console.log('Saved!');
  });
}
...
function loadDatabase(){
  fs.readFile('./databasestorage/saveddatabase.yml', 'utf8', (err, data) => {
    if (err) throw err;
    if(data){
      database = yaml.load(data);
      console.log('Loaded!');
      //return blah; //<--- Note: based on the pre-written code that calls loadDatabase it looks like they want this function to return the database object, however I think there is a bug having something to do with asynchronus stuffs, so I'm just setting the database object in the line above instead
    }else{
      database = {
        users: {},
        articles: {},
...
//The following commented out chunk is a code graveyard from failed attempts using node-persist. I'm leaving it here just as a trail of my thinking and in case I want to go back to it
...
//  await storage.init(persistOptions);
//  await storage.setItem('thescoop', database, persistOptions).then(() => {
//      return storage.getItem('thescoop');
//  }).then(value => {
    //this part doesn't really work, it doesn't write the user name just [object Object]...super annoying
//    console.log(`stored value is ${value.users.username}`)
//  });
//}

//note: this function doesnt work, it keeps saying storage.getItem is not a function...doesn't make any sense
//async function loadDatabase(){
//  try{
//    console.log('>>>>>>>>> loaded');
//    let retrieved = await storage.getItem('thescoop');
...
//note: I don't think this code is doing what it was intended to do, savedDatabse returns falsey and therefore never writes database object. I think this has something to do with asynchronous code but what do I know
console.log('------Kickoff loadDatabase');
if (typeof loadDatabase === 'function' && !isTestMode) {
  console.log('------loadDatabase is a functon and we are not in TestMode');
  const savedDatabase = loadDatabase();
  if (savedDatabase) {
    console.log('------savedDatabase is valid');
    for (key in database) {
      database[key] = savedDatabase[key] || database[key];
    }
  }
  console.log('------savedDatabase is NOT valid');
}
console.log('-------Finish loadDatabase');
